...
parameter vector whose jth element $p_j$  is the mass of the $j$th equivalent source 
(point mass) at the jth Cartesian coordinates ($x_j$,$y_j$,$z_0$) and $\mathbf{A}$ 
...
\begin{equation}
a_{ij}= c_{g} \frac{G \, (z_{0} - z_{i}) \, 10^{-5}}{\left[(x_{i} - x_{j})^{2} +
(y_{i} - y_{j})^{2} +	(z_{i} - z_{0})^{2} \right]^{\frac{3}{2}}} \; .
\label{eq:aij}
\end{equation}

where $G$ is the Newton's gravitational constant and $c_{g} = 10^5$ is a constant transforming from $\mathrm{m/s^2}$ to mGal.
