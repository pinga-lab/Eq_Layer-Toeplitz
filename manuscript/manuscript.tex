...
in terms of another pair of grid coordinates $x_{k}$ and $y_{l}$.
...
Equations \ref{eq:xk} and \ref{eq:yl} show that the grid coordinates $x_{k}$ and $y_{l}$ 
are functions of the grid indices $k$ and $l$.

In the same way, the matrix coordinates $x_{i}$, $y_{i}$, $x_{j}$ and $y_{j}$ are defined
in terms of the matrix indices $i$ and $j$. However, the relationship between these
quantities are more complicated than those for grid coordinates 
(equations \ref{eq:xk} and \ref{eq:yl}).

In order to investigate the structure of matrix $\mathbf{A}$ (equation \ref{eq:predicted-data-vector}),
it is necessary to define equations 

for the case in which the data 
are disposed at a regular grid on a horizontal plane, we analyze the relationship 
between the matrix and grid coordinates/indices.
...
$x$-axis (Figure XXa). For convenience, we designate these grids as $x$-\textit{oriented grids}.
For them, the relationship between the matrix and grid indices are defined
...
Consider the case in which the regular grid of observation points is oriented along the
$y$-axis (Figure XXb). For convenience, we call them $y$-\textit{oriented grids}.
...
written in terms of the grid indices $k$ and $l$ by properly using 
equations \ref{eq:i-x-oriented}, \ref{eq:l-x-oriented}, \ref{eq:i-y-oriented} and \ref{eq:k-y-oriented},
as follows:


\subsubsection{Structure of matrix $\mathbf{A}$}
...
Since this matrix is Block Toeplitz, it is convenient to label the 
