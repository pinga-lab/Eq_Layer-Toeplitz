...
\citep[][ p. 185]{davis1979}. It means that $\mathbf{C}$ (equation \ref{eq:auxiliary-system}) 
satisfies 
\begin{equation}
\mathbf{C} = 
...
\end{equation}
where the symbol ``$\otimes$" denotes the Kronecker product \citep[][ p. 22]{davis1979},
...
\citep[][ p. 31]{jain1989} and the symmetry of unitary DFT matrices (see Appendix A),
...
\append{Kronecker product for separable transformations}

In the present Appendix, we deduce equation \ref{eq:DFT-system} by applying the 
row-ordered $vec$-operator, here designated simply as $vec$-operator.
...
transformations. The $vec$-operator used here 
...
What follows is based on \citet{jain1989}.

Let $\mathbf{M}$ be an arbitrary $2Q \times 2P$ matrix given by:
\begin{equation}
...
\label{eq:matrix-M}
\end{equation}
where $\mathbf{m}_{i}$, $i = 1, \dots, 2Q$, are $2P \times 1$ vectors containing 
...
\vdots \\
\mathbf{m}_{2P}
\end{bmatrix}_{4QP \times 1} \: .
\label{eq:vec-operator}
\end{equation}
This rearrangement is known as lexicographic ordering \citep[][ p. 150]{jain1989}.

Now, the two-dimensional DFT of $\mathbf{M}$ (equation \ref{eq:matrix-M}) be
defined in matrix notation as follows:
\begin{equation}
\tilde{\mathbf{M}} = \mathbf{F}_{2Q} \, \mathbf{M} \, \mathbf{F}_{2P} \: ,
\label{eq:2D-DFT-M}
\end{equation}
where $\mathbf{F}_{2Q}$ and $\mathbf{F}_{2P}$ are the $2Q \times 2Q$ and $2P \times 2P$ 
unitary DFT matrices. This equation shows that the two-dimensional DFT is a 
separable transformation \citep[][ p. 17]{jain1989}.
By implicitly using the $vec$-operator (equation \ref{eq:vec-operator}), 
\citet[][ p. 31]{jain1989} showed that:
\begin{equation}
vec \left( \tilde{\mathbf{M}} \right) = 
\left( \mathbf{F}_{2Q} \otimes \mathbf{F}_{2P}^{\top} \right) 
vec \left( \mathbf{M} \right) \: .
\label{eq:kronecker-separable}
\end{equation}
This equation is valid not only for the two-dimensional DFT, but for any separable 
transformation defined with arbitrary matrices other than the unitary DFT matrices
$\mathbf{F}_{2Q}$ and $\mathbf{F}_{2P}$ (equation \ref{eq:2D-DFT-M}).
We stress that equation \ref{eq:kronecker-separable} is different from that 
presented by \citet{neudecker1969}. While equation \ref{eq:kronecker-separable} is 
defined by using the $vec$-operator shown in equation \ref{eq:vec-operator}, that presented 
by \citet{neudecker1969} is defined by using a $vec$-operator that 
transforms a matrix into a column vector by stacking its columns.

Consider that vector $\mathbf{w}$ (equation \ref{eq:auxiliary-system-manipulated}) 
is obtained by applying the $vec$-operator (equation \ref{eq:vec-operator}) to a matrix 
$\mathbf{W}$, whose two-dimensional DFT is represented by $\tilde{\mathbf{W}}$, 
and remember that unitary DFT matrices are symmetric. In this case, 
according to equations \ref{eq:2D-DFT-M} and \ref{eq:kronecker-separable}, we can rewrite 
the right side of equation \ref{eq:auxiliary-system-manipulated} as follows:
\begin{equation}
...
\end{equation}
Similarly, consider that $\mathbf{v}$ (equation \ref{eq:auxiliary-system-manipulated}) 
is obtained by applying the $vec$-operator (equation \ref{eq:vec-operator}) to a matrix 
$\mathbf{V}$, whose two-dimensional DFT is represented by $\tilde{\mathbf{V}}$, and 
use equations \ref{eq:2D-DFT-M} and \ref{eq:kronecker-separable} to rewrite the 
left side of equation \ref{eq:auxiliary-system-manipulated} as follows:
\begin{equation}
...
\end{equation}
Because $\boldsymbol{\Lambda}$ is a diagonal matrix, 
equation \ref{eq:left_side_DFT_system_1} 

PAREI AQUI

Using the fact that $\boldsymbol{\Lambda}$ is a diagonal matrix, we can 
rewrite equation \ref{eq:left_side_DFT_system_1} 


Additionally, premultiply the result by the diagonal matrix $\boldsymbol{\Lambda}$ 


Notice that the left side of equation \ref{eq:auxiliary-system-manipulated} is 
premultiplied by the diagonal matrix $\boldsymbol{\Lambda}$. 
