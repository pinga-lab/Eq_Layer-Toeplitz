...
Therefore, at each iteration $k$th of the fast equivalent-layer technique, 
...
of each block $\mathbf{A}_{q}$ (equation \ref{eq:Aq_block}) forming the BTTB matrix 
$\mathbf{A}$ (equation \ref{eq:BTTB_A});

\item[\textbf{(2)}] Rearrange the first column of $\mathbf{A}$  equations


$\mathbf{c}^{q}_{0}$ 
(equation \ref{eq:Cq-first-column}) of each block $\mathbf{C}_{q}$ to obtain the 
