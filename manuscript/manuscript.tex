...
(equivalent sources) over a layer located at depth $z_o$ ($z_o >z_i$) and whose 
...
where $\mathbf{d}$ is an $N \times 1$ vector whose ith element is the predicted gravity 
data at the ith point ($x_i$,$y_i$,$z_i$), $\mathbf{p}$ is the unknown $N \times 1$ 
parameter vector whose jth element $p_j$  is the mass of the jth equivalent source 
(point mass) at the jth Cartesian coordinates ($x_j$,$y_j$,$z_o$) and $\mathbf{A}$ 
is an $N \times N$  sensitivity matrix whose ijth element is given by 
...
Notice that the sensitivity matrix depends on the ith coordinates of the observations 
and the jth coordinates of the equivalent sources. For convenience, we designate 
these coordinates as matrix coordinates and the indices i and j as matrix indices.

In the classical equivalent-layer technique, we estimate the regularized parameter vector from the observed gravity data $d^O$ by
