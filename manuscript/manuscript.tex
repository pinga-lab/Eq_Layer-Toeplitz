...
At each iteration of the fast equivalent-layer technique, our method computes 
one DFT, one Hadammard product and one IDFT involving $2Q \times 2P$ matrices 
(left side of equation \ref{eq:DFT-system}). 

The Hadamard product requires $4N$ flops.

Let us consider that the total number of flops required to compute a DFT/IDFT 
by using a 2D FFT is $\kappa \, 4N \log_{2}(4N)$, where $N = QP$ and $\kappa$ is a constant
depending on the algorithm. 

Then, the resultant flops count of our method is given by:

\begin{equation}
f_1 = N^{it} \left[ 7N + \kappa \, 8N \log_{2}(4N) \right].
\label{eq:float_bccb}
\end{equation}
